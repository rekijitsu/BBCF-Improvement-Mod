...
- Adds an in-game Jukebox that rotates BGM during matches: sequential or shuffle rotation with per-track true-length auto-advance (tracks play to the end, and the music never stops on its own - astral music interruption works), Repeat Single, per-category enable/disable, track search, click-to-play - toggle with the Jukebox hotkey (default `F6`)
